...
int main(int argc, char *argv[]) {
  ...
  //while (true) 
  {
    ...
    if (runConfig.outputType == OUTPUT_DIRECTORY) {
      ...
      piper::textToWavFile(piperConfig, voice, trans_text, audioFile, result);
      ...
      piper::textToWavFile(piperConfig, voice, trans_text, cout, result);
      ...
      piper::textToAudio(piperConfig, voice, trans_text, audioBuffer, result,
                         audioCallback);
...
void parseArgs(int argc, char *argv[], RunConfig &runConfig) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (arg == "-m" || arg == "--model") {
      ...
      ensureArg(argc, argv, i);
      trans_text = filesystem::path(argv[++i]);
